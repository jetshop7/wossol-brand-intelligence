...
The Product Route / Backend Coverage review found that `/shopify` page counting understated an embedded app primarily implemented through a route response, static App Home script, Shopify extension assets and backend services. In response, `SHOPIFY_EMBEDDED_APP_COD_COMMERCE_EXPERIENCE.md` first documents the exact committed Product snapshot `4e26b4369e6416c22c731b8be706d72562a19d5b`, its connected authority/Order boundaries, cross-section implications and focused tests. The subsequent Director review required a targeted `4e26b436` → `97f9959` Shopify-only delta verification; supplement Section 10 records the preflight contract, customer validation, Upsell target-Variant exclusivity, storefront flow and focused test outcomes. Product HEAD was `fd7157f...` at that follow-up, with Shopify files unchanged from `97f9959`; intervening/later Advertising and Messaging work is outside the Shopify evidence. The two earlier failures persist, including an unresolved fixed-Variant presentation contract. The supplement is not Director acceptance; Master Synthesis remains pending Director re-review.
